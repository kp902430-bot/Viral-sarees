...
import { Crown, Lock, ShieldCheck, ArrowRight, X, AlertCircle } from 'lucide-react';
...
export const OwnerAuthModal: React.FC<OwnerAuthModalProps> = ({
  ...
  const handlePinSubmit = (e: React.FormEvent) => {
    ...
    if (cleanPin === '7990' || cleanPin === '799065' || cleanPin === '9024' || cleanPin === '902430' || cleanPin === 'admin123' || cleanPin === 'vls2026') {
      ...
      onSuccess(session);
      onShowToast('👑 Store Owner Verified! Welcome to Merchant Dashboard.');
      onClose();
    } else {
      ...
      if (result.success && result.user) {
        ...
        if (userEmail === 'kamal799065@gmail.com' || userEmail === 'kp902430@gmail.com') {
          const session: OwnerSession = {
            isLoggedIn: true,
            email: userEmail,
            name: result.user.displayName || 'Store Owner',
            role: 'Store Owner',
            loginTime: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })
          };
          onSuccess(session);
          onShowToast(`👑 Welcome back! Merchant controls unlocked.`);
          onClose();
        } else {
          setError(`Access denied. This Google account does not have merchant permissions.`);
        }
      } else {
        setError(result.error || 'Google Sign-In was cancelled.');
      }
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-xs animate-fadeIn">
      <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden border border-amber-300">
        ...
        <div className="bg-gradient-to-r from-[#2B050B] via-[#4A0A16] to-[#2B050B] text-amber-100 p-6 relative">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-lg font-bold text-white">Merchant Control Center</h3>
              <p className="text-xs text-amber-200/80">Authorized Store Access</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="p-3.5 bg-stone-50 border border-stone-200 rounded-2xl text-xs space-y-1 text-stone-800">
            <div className="flex items-center gap-1.5 font-bold text-stone-900">
              <ShieldCheck className="w-4 h-4 text-emerald-700" />
              <span>Confidential Merchant Verification</span>
            </div>
            <p className="text-[11px] text-stone-500">
              Sign in with your authorized credentials to manage inventory and orders.
            </p>
          </div>
          ...
          <div>
            ...
            >
              ...
              <span>{isLoadingGoogle ? 'Connecting...' : 'Sign in with Google'}</span>
            </button>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••"
                  maxLength={10}
                  className="w-full pl-9 pr-3 py-2.5 bg-stone-50 border border-stone-300 rounded-xl text-base font-mono tracking-widest focus:ring-2 focus:ring-amber-500 focus:bg-white outline-hidden"
                  autoFocus
                />
              </div>
            </div>
